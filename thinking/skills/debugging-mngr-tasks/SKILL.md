...
- `mngr list --active --format jsonl` - see all running agents and their states (and exclude all archived agents and yourself from the list so you can focus on the agents that matter)
...
Start by running `mngr list --active --format jsonl` to see all currently running agents (besides yourself) and their states, or `mngr capture <agent> --full` to see the full output from a given agent.
