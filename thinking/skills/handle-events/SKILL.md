...
Note: this skill also applies to warning and error events from sources that don't have their own handler. If you receive a warning or error from an unknown source, follow the same procedure above -- the user should be informed about unexpected warnings and errors, even if you subsequently ignore that source.
