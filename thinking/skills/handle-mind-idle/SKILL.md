...
5. **Proactive work**: If nothing above needs attention and you have no agents in flight, consider whether there's something useful you could do proactively:
   - **Triage untriaged messages**: Use `list-channel-unread` or `list-channel-importance` to see if there are unread messages that haven't been triaged yet. If so, consider running `backfill-slack-triage` on the most important channels.
