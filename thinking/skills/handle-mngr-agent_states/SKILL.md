...
```bash
cat $MNGR_HOST_DIR/agents/<agent-id>/outputs/summary.md 2>/dev/null || echo "No summary.md found"
```
...
You can determine the agent's role and associated ticket by checking its labels (e.g., `mngr list --active --format jsonl | grep <agent-id> | jq .labels` and look at the `role` and `ticket` labels).
...
This will create a verifying agent to review the work. Be sure to pass the ticket ID (from the agent's `ticket` label) to `create_verifying_agent.sh` so the verifier also carries the link.
