...
**Authentication (for service integrations)**: If the workflow involves external services, scripts should use `latchkey curl <service> <url>` for authenticated API calls. If a failure is auth-related, check whether latchkey itself is working (`latchkey curl <service> <test-url>`) before modifying the script.
